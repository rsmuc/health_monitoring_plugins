...
# along test_check_snmp_lband.py.  If not, see <http://www.gnu.org/licenses/>.

import os
import sys
sys.path.insert(0, os.path.abspath('health_monitoring_plugins/check_snmp_lband'))
from check_snmp_lband import *
...
def test_start():
    # start the testagent
    walk =  '''.1.3.6.1.4.1.31210.52.1.9.0 = INTEGER: 0            
            .1.3.6.1.4.1.31210.52.1.13.0 = INTEGER: 0'''   
    register_snmpwalk_ouput(walk)
    start_server()
  
def test_Lband_basics():
  #without options
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_lband/check_snmp_lband.py", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  myout =  p.stdout.read()
  print "--------------------"
  print "got"
  print myout
...
def test_ok_outputs():
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_lband/check_snmp_lband.py -H 127.0.0.1:1234", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  myout =  p.stdout.read()
  print "--------------------"
  print "got"
  print myout
...
def test_ok():
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_lband/check_snmp_lband.py -H 127.0.0.1:1234 -U 1", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  assert "OK - Unit status is: Online" in p.stdout.read()
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_lband/check_snmp_lband.py -H 127.0.0.1:1234 -U 2", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  assert "OK - Unit status is: Online" in p.stdout.read()
  
def test_start_critical():
    unregister_all()
    walk =  '''.1.3.6.1.4.1.31210.52.1.9.0 = INTEGER: 2            
            .1.3.6.1.4.1.31210.52.1.13.0 = INTEGER: 2'''  
