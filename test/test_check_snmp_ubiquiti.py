...
# test_check_ubiquiti.py - test for Monitor a Ubiquity airMax.
...
def test_ubiquiti_basics1():
  ...
  #without options
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_snmp_ubiquiti/check_snmp_ubiquiti.py", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  myout =  p.stdout.read()
  print "--------------------"
  print myout
  ...
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_snmp_ubiquiti/check_snmp_ubiquiti.py -H 1.2.3.4", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  assert "Unknown - Argument -t is missing or false!" in p.stdout.read()
  
   # with  unknown host (-H 1.2.3.4) and -t qL1  
  p = subprocess.Popen("health_monitoring_plugins/check_ubiquiti/check_ubiquiti.py -H 1.2.3.4 -t up", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def test_ubiquiti_ok_outputs():
  for n,u in zip(names,units):
    p = subprocess.Popen("health_monitoring_plugins/check_snmp_snmp_ubiquiti/check_snmp_ubiquiti.py -H 127.0.0.1:1234 -t "+n +" --th metric=type,warning=:10,critical=:20", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def test_uptime():
  ...
  register_snmpwalk_ouput(walk_var)
  p = subprocess.Popen("health_monitoring_plugins/check_snmp_snmp_ubiquiti/check_snmp_ubiquiti.py -H 127.0.0.1:1234 -t \"up\" --th metric=type,warning=:10,critical=:20", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
