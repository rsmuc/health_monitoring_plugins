...
def test_start():
 # start the testagent
    ...
    register_snmpwalk_ouput('''iso.3.6.1.4.1.674.10892.5.4.700.20.1.8.1.3 = STRING: "CPU 2"''')# location name of the temperature probe
...
def test_everything_not_ok():
 ...
 Cooling unit "System Board Fan Redundancy" status: critical. Temperature probe at "System Board Inlet Temp" is\
 criticalUpper.' in cmd_output
...
def test_temperature_probe_critical():
    """
    temperature probe 'CPU 1' status critical
    """
    
    temperature_summary, temperature_long = temperature_probe_check([3,5,3],['Inlet Temp','CPU 1','CPU 2'])
    
    print 'summary: %s' % temperature_summary
    print 'long: %s' % temperature_long
    
    assert 'Temperature probe at "CPU 1" is critical' in temperature_summary
    assert 'Temperature probe at "CPU 1" is critical' in temperature_long
